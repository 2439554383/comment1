...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    private MethodChannel overlayMessageChannel1;
    ...
    private static final String CHANNEL = "com.example.message1";
    ...
    private void resizeOverlay(double width, double height, boolean enableDrag, MethodChannel.Result result) {
        if (windowManager != null && flutterView != null) {
            try {
                WindowManager.LayoutParams params = (WindowManager.LayoutParams) flutterView.getLayoutParams();
                if (params == null) {
                    Log.e("OverlayService", "resizeOverlay: params 为 null");
                    result.error("RESIZE_ERROR", "LayoutParams 为 null", null);
                    return;
                }
                ...
                result.error("RESIZE_ERROR", "调整大小失败: " + e.getMessage(), null);
            }
        } else {
            Log.e("OverlayService", "resizeOverlay: windowManager 或 flutterView 为 null");
            result.success(false);
        }
    }
    ...
    public void onCreate() {
        ...
        if (flutterEngine != null) {
            ...
            overlayMessageChannel1 = new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(), CHANNEL);
            overlayMessageChannel1.setMethodCallHandler(
                    (call, result) -> {
                        if (call.method.equals("printMessage")) {
                            String message = call.argument("msg");
                            System.out.println("收到overlay消息: " + message);
                            try {
                                ClipboardManager clipboard = (ClipboardManager) getApplicationContext().getSystemService(Context.CLIPBOARD_SERVICE);
                                ClipData clip = ClipData.newPlainText("Flutter Message", message);
                                clipboard.setPrimaryClip(clip);
                                Log.i("OverlayService", "已复制到剪贴板: " + message);
                                Toast.makeText(this, "复制成功", Toast.LENGTH_SHORT).show();
                            } catch (Exception e) {
                                Log.e("OverlayService", "复制失败", e);
                                Toast.makeText(this, "复制失败", Toast.LENGTH_SHORT).show();
                            }
                            result.success(null);
                        }
                        else if (call.method.equals("get_board")) {
                            try {
                                ClipboardManager clipboard = (ClipboardManager) getApplicationContext().getSystemService(Context.CLIPBOARD_SERVICE);
                                ...
                                if (clipboard.hasPrimaryClip()) {
                                    ...
                                    Log.i("OverlayService", "获取到剪贴板内容: " + text);
                                    result.success(text); // 将内容返回给Flutter
                                } else {
                                    result.success(""); // 返回空字符串表示剪贴板无内容
                                }
                            } catch (Exception e) {
                                Log.e("OverlayService", "获取剪贴板失败", e);
                                result.error("CLIPBOARD_ERROR", e.getMessage(), null);
                            }
                        }
                        else {
